...
public class CommandForJavaHome {
    ...
    // 无参构造方法时获取各盘下 Program Files 和 Program Files (x86) 下 Java 文件夹（不一定存在）内的 jre 或
    // jdk 地址
    CommandForJavaHome() {
    ...
    public void SetJavaHome(int commandnum, String input) {
        ...
        if (input.length() == 0 || input.charAt(0) != ' ' || input.substring(1).length() == 0
                || !isInAllJava(input.substring(1))) {
