...
public class CommandForManage {
    ...
    public static void showInfo(String[][] input) {
        ...
        String[] Title = new String[] { "Num", "Name", "Age", "Address", "Class", "Math_Score", "English_Score",
                "Politics_Score", "Major_Score", "Total_Score", "Student_Kind", "Major", "SenSei", "Target" };
        Infos[0] = Title;
    ...
    public void CreateStudent(int commandnum, String[] splitofcommand) {
        ...
        if (Objects.equals(splitofcommand[1], "Honkasei")) {
            ...
            if (searchIndexOf(num) != -1) {
                System.out.printf(
                        "\33[31;1mStudent Num.%d already exists, please change the num of student you want to create!\33[0m\n\n",
                        num);
                return;
            }
    ...
    public void SearchStudent(int commandnum, String[] splitofcommand) {
        ...
        // 按照 /search student
        // with（Num.<student_num>|Name.<student_name>|Class.<student_year>_<student_class>)
        // 的描述
        // 现在输入的部分应当满足
        // (Num.<student_num>|Name.<student_name>|Class.<student_year>_<student_class>)
        // 的格式
        if (splitofcommand.length != 2) {
            // 不满足格式直接 illegal 处理，再把对应的帮助怼到用户脸上
            System.out.print("\33[31;1mThis command is illegal!\33[0m\n");
        ...
        if (Objects.equals(split[0], "Num")) {
            ...
            try {
                ...
                System.out.printf(
                        "\33[31;1mStudent Num.%s doesn't exists, please check the num of student you want to search!\33[0m\n\n",
                        split[1]);
                return;
            }
            ...
            if (searchIndexOf(num) == -1) {
                // 不存在则提示
                System.out.printf(
                        "\33[31;1mStudent Num.%d doesn't exists, please check the num of student you want to search!\33[0m\n\n",
                        num);
                return;
            }
            ...
            if (infosindex == 0) {
                // 不存在则提示
                System.out.printf(
                        "\33[31;1mStudent Name.%s doesn't exists, please check the name of student you want to search!\33[0m\n\n",
                        split[1]);
            } else {
                // 反之显示即可
                String[][] Infos = new String[infosindex][14];
                for (int i = 0; i < infosindex; i++) {
                    if (Objects.equals(students[i].name, split[1])) {
            ...
            try {
                ...
                System.out.printf(
                        "\33[31;1mStudent Class.%s_%s doesn't exists, please check the class of student you want to search!\33[0m\n\n",
                        kurasu[0], kurasu[1]);
                return;
            }
            ...
            if (infosindex == 0) {
                // 不存在则提示
                System.out.printf(
                        "\33[31;1mStudent Class.%d_%d doesn't exists, please check the class of student you want to search!\33[0m\n\n",
                        year, class_num);
            } else {
                // 反之显示即可
                String[][] Infos = new String[infosindex][14];
    ...
    public void SetStudent(int commandnum, String[] splitofcommand) {
        ...
        if (indexofstudent == -1) {
            System.out.printf(
                    "\33[31;1mStudent Num.%d doesn't exists, please check the name of student you want to search!\33[0m\n\n",
                    num);
            return;
        }
    ...
    public void ShowStudent(int commandnum, String[] splitofcommand) {
        ...
        if (splitofcommand.length != 4 || !Objects.equals(splitofcommand[2], "with")
                || !(Objects.equals(splitofcommand[3], "up") || splitofcommand[3].equals("down"))) {
            // 不满足格式直接 illegal 处理，再把对应的帮助怼到用户脸上
            System.out.print("\33[31;1mThis command is illegal!\33[0m\n");
            System.out.print("Check by this: \33[31;1m" + Help[commandnum] + "\33[0m\n\n");
            return;
        ...
        for (int i = 0; i <= 5; i++) {
            if (Objects.equals(splitofcommand[1], Match[i])) {
                ...
                if (i != 5) {
                    ...
                    // 还需选择
                    System.out.print(
                            "Please enter the lesson name (Math|English|Politics|Major|Total) of the score you want to query: ");
                    Scanner input = new Scanner(System.in);
                    input.close();
    ...
    public void ShowStatistic() {
        ...
        int kensyuusei = 0;
        for (int i = 0; i < studentIndex; i++) {
            if (Objects.equals(students[i].kind, "Honkasei")) {
                honkasei++;
            }
            if (Objects.equals(students[i].kind, "Kensyuusei")) {
                kensyuusei++;
            }
        }
        System.out.printf("There are \33[32;1m%d\33[0m Honkasei(s)\n", honkasei);
        System.out.printf("There are \33[32;1m%d\33[0m Kensyuusei(s)\n", kensyuusei);
        System.out.printf("There are \33[32;1m%d\33[0m Student(s)\n\n", studentIndex);
    }
}
